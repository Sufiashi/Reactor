...
private:
    ...
    std::function<void()> errorcallback_;        //fd发生错误的回调函数，将回调Connection::errorcallback()
    std::function<void()> writecallback_;        //fd写事件的回调函数，将回调Connection::writecallback()
public:
    ...
    void setclosecallback(std::function<void()> fn);    // 设置关闭fd的回调函数。
    void seterrorcallback(std::function<void()> fn);    // 设置fd发生错误的回调函数。
